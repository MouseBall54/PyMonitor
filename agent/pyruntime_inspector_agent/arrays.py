...
def describe(value, summarize):
    ...
    return {
        ...
        "supportedPreviewModes": ["GRAY", "HWC", "CHW"] if value.dtype.kind == "u" and value.itemsize == 1 else [],
    }


def preview(value, max_width=1024, max_height=1024, layout=None):
...
def pixel(value, coordinates, layout=None):
    _require_uint8_array(value)
    ...
    return {"coordinates": [y, x], "dtype": str(value.dtype), "layout": selected_layout, "value": raw}
