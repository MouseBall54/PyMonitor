...
| Light professional shell, optional Dark theme, semantic states, and accessibility labels | Partially proven | The current primary display reports 2560×1440 at 96 DPI (100%), where the Release captures show readable Light hierarchy, selection, empty state, command bar, status bar, and automation names. WPF smoke tests fail on binding warnings and exercise a 960×540 logical viewport with 5,000 virtualized rows. Per-monitor-v2 metadata is verified. Physical 125%, 150%, and 200% display scaling is still a manual gate. |
