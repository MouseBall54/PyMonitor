...
- MSI 업데이트: 더 높은 버전의 MSI를 받아 SHA-256을 확인한 뒤 실행하면 기존
  PyMonitor를 감지해 제자리 major upgrade합니다. 자동 업데이트 기능은 없으며 더
  낮은 버전으로의 downgrade는 차단됩니다.
- Portable 업데이트: 새 ZIP을 새 폴더에 풀어 실행을 확인한 뒤 이전 버전 폴더를
  삭제합니다. 새 파일을 기존 폴더 위에 덮어써 섞지 않는 것이 안전합니다.
...
Agent suite 97개는 CPython 3.10.18, 3.11.9, 3.12.9, 3.13.7,
3.14.0rc2에서 NumPy·pandas·Matplotlib을 함께 설치한 상태로 통과했습니다
(런타임별 비지원 기능은 예상대로 skip). .NET Release suite는 App 80개,
Protocol 5개, Integration 2개로 총 87개가 통과했습니다. VS Code
