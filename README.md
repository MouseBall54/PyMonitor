...
| --- | --- |
| 변수 찾기 | frame의 locals/globals, module·내부 콘솔 namespace와 GC-tracked object를 이름·타입·값으로 검색하고 필터링 |
| 객체 안쪽 탐색 | 선택한 변수에서 시작해 collection, mapping, instance field를 Object Tree로 이동하고 class, MRO, method와 parameter를 정적으로 확인 |
| 전체 런타임에서 찾기 | Global Search에서 module·frame 변수, 재귀 객체 경로, instance, class, method와 property를 통합검색하고 정확한 위치를 열기 |
...
**Global Search**는 현재 선택한 Variables 표만 필터링하는 검색과 다릅니다. 연결된
Python의 모든 로드된 module namespace, 탐지된 내부 콘솔 namespace와 thread frame의
locals, globals, built-ins 및 GC-tracked objects를 시작점으로 변수·객체·class
정보를 함께 찾습니다.

- 변수 이름, type, module, qualified type, 안전한 값 preview, 주소와 전체 객체 경로
...
Locals / value...` 형식의 전체 위치가 표시됩니다. 결과를 두 번 클릭하거나 **Open
...
읽을 수 있는 정적 참조의 객체 identity만 비교합니다. 앱의 Global Search는 고정된
객체 수·깊이·child·edge·시간 한도 없이 그래프를 끝까지 탐색하며, 순환·공유 객체는
동일 하위 그래프를 중복 확장하지 않습니다. 검색 결과 표와 프로토콜 응답 크기는
계속 제한되므로 결과 한도, 동시 변경 또는 응답 크기 한도가 표시되면 결과 목록이
완전하지 않을 수 있습니다. 큰 runtime에서는 검색이 오래 걸릴 수 있으며 **Cancel**로
대기 중인 검색을 취소할 수 있습니다.
단, CPython의 interpreter-wide audit hook은 `gc.get_objects`와
`sys._current_frames` snapshot 이벤트를 관찰하거나 거부할 수 있습니다.

## 프로그램 내부 콘솔 변수

Runtime Tree의 **Console namespaces**는 같은 Python 프로세스 안에서 별도 namespace를
사용하는 내장 콘솔 변수를 직접 Variables source로 보여 줍니다. 표준
`code.InteractiveConsole`/`InteractiveInterpreter`, IPython의 정적 `_user_ns`, 그리고
class 이름에 `console`, `interactive`, `repl`, `shell`, `terminal`이 포함되고
`locals`, `namespace`, `user_ns` 같은 exact `dict` instance field를 가진 콘솔을
읽기 전용으로 자동 탐지합니다. 한 번 선택하면 일반 scope와 같이 자동 새로 고침과
Added/Removed/Rebound/Updated 비교가 적용됩니다.

임의의 `exec(code, namespace)` 딕셔너리는 실행 후 콘솔이라는 표지가 남지 않으므로
명시적으로 등록할 수 있습니다.
...
terminal_namespace = {}
registration_id = register_namespace("내장 터미널", terminal_namespace)

# 애플리케이션의 콘솔이 이 namespace에 변수를 선언합니다.
exec("threshold = 0.75", terminal_namespace)

# 콘솔 수명이 끝날 때 등록이 잡고 있던 강한 참조를 해제합니다.
unregister_namespace(registration_id)
```

자동 탐지는 연결/Runtime Refresh 때 GC를 강제 수집하지 않고 최대 100,000개의
GC-tracked owner를 검사합니다. 한도에 걸리면 Runtime Tree에 incomplete 상태를
표시합니다. 별도 subprocess의 콘솔은 메모리 공간이 다르므로 그 프로세스에 따로
attach해야 합니다. 자세한 계약은 [Console namespaces](docs/console-namespaces.md)를
참고합니다.
...
PyMonitor의 기본 정보 구조는 `Inspect`, `Global Search`, `Launch`, `Memory`,
`Events` 다섯 탭입니다.
...
연결됩니다. Runtime Tree에서 frame scope, 이미 로드된 module namespace, 내부
console namespace 또는 GC-tracked objects를 고르고 Variables에서 한 행을 선택하면, 오른쪽의 같은
...
Object Tree, Class and Methods 또는 데이터 전용 탭에서 객체 history를 이동해도
현재 상세 탭을 유지하므로 Back / Forward 탐색의 흐름이 끊기지 않습니다.
...
- [Embedded Console Namespaces](docs/console-namespaces.md)
...
agent/
  pyruntime_inspector_agent/
    ...
    console_namespaces.py        # embedded console namespace discovery/registration
...
- `InteractiveConsole`/IPython/custom/registered console namespace 변수와 변경 추적
...
- 내부 콘솔 자동 탐지도 최대 100,000개의 GC-tracked owner로 제한되며, 임의의 별도
  `exec` 딕셔너리는 `register_namespace`로 등록해야 합니다. 등록은 명시적으로
  해제할 때까지 namespace를 강하게 참조합니다.
