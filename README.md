...
Python의 모든 로드된 module namespace와 thread frame의 locals, globals,
built-ins 및 GC-tracked objects를 시작점으로 변수·객체·class 정보를 함께 찾습니다.
...
state...` 또는 `Threads / <id> / <frame> / Locals / value...` 형식의 전체 위치가
표시됩니다. 결과를 두 번 클릭하거나 **Open location**을 누르면 해당 객체를 Inspect에
열며 class/member 결과는 **Class and Methods**의 해당 이름까지 이동합니다.
